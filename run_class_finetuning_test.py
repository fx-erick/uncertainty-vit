...
def main(args, ds_init):
    ...
    if args.disable_eval_during_finetuning:
        ...
        dataset_train, dataset_val = torch.utils.data.random_split(dataset_train, [[train_set_size, valid_set_size]])
    ...
    if args.eval:
        ...
        print(f"Accuracy and of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}% , ECE : {test_stats['ECE']:.1f}, NLL : {test_stats['NLL']:.1f}")
        exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            test_stats = evaluate(data_loader_val, model, device)
            print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%, , ECE : {test_stats['ECE']:.1f}, NLL : {test_stats['NLL']:.1f}")
            ...
            print(f'Max accuracy: {max_accuracy:.2f}%,ECE : {ECE:.2f},NLL : {NLL:.2f}')
            if log_writer is not None:
    ...
    if args.mc_dropout_forwards != 0:
        ...
        evaluate_MC_dropout(data_loader_val, model, device, args.mc_dropout_forwards)
