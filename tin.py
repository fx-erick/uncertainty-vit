...
class TinyImageNetDataset(Dataset):
  def __init__(self, root_dir, mode='train', preload=False, load_transform=None,
               transform=None, target_transform = None, download=False, max_samples=None):
    ...
    if self.preload:
      ...
      for idx in tqdm(range(self.samples_num), desc=load_desc, disable=True):
        s = self.samples[idx]
...
class TinyImageNetCPaths:
  def __init__(self, root_dir, download=False):
    if download:
      download_and_unzip('http://cs231n.stanford.edu/tiny-imagenet-200.zip',
                         root_dir)

    wnids_path = '/home/atuin/b143dc/b143dc20/datasets/tiny-imagenet-c/wnids.txt'
    words_path = '/home/atuin/b143dc/b143dc20/datasets/tiny-imagenet-c/words.txt'

    self._make_paths(root_dir, wnids_path, words_path)

  def _make_paths(self, train_path, wnids_path, words_path):
    self.ids = []
    with open(wnids_path, 'r') as idf:
      for nid in idf:
        nid = nid.strip()
...
class TinyImageNetC(Dataset):
  def __init__(self, root_dir,preload=False, transform=None, target_transform=None, download=False, max_samples=None):
    tinp = TinyImageNetCPaths(root_dir, download)
    self.label_idx = 1  # from [image, id, nid, box]
    self.preload = preload
    self.transform = transform
    self.transform_results = dict()
